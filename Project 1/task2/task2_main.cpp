...
void bilinear_expand(my_image_comp *in, my_image_comp *out)
{
  // Perform the convolution
  for (int r2 = 0, r1 = 0; r2 < out->height; r2++)
    for (int c2=0; c2 < out->width; c2++)
      {
        // interpolarate 2 points between 2 input pixel;
        int r1 = r2 / 3.0F;
        int c1 = c2 / 3.0F;
...
int main(int argc, char *argv[])
{
  ...
  try {
      ...
      // Process the image, all in floating point
      for (n=0; n < num_comps; n++)
        input_comps[n].perform_boundary_extension();
      clock_t start_time = clock();
      for (n = 0; n < num_comps; n++) {
          printf("Bilinear interpolation start!\n");
          bilinear_expand(input_comps + n, output_comps + n);
      }
